...
namespace OtelRezerved_Network
{
    ...
    public partial class giris_kayit_Form : Form
    {
        ...
        public static class TextYazi
        {
        public static string Yazi { get; set; }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(giris_email_textBox.Text) && !string.IsNullOrEmpty(kayit_adsoyad_textBox.Text))
            {
                TextYazi.Yazi = kayit_adsoyad_textBox.Text;
            }
            else
            {
                TextYazi.Yazi = giris_email_textBox.Text;
            }
            ...
            using (OleDbConnection baglanti = new OleDbConnection("Provider=Microsoft.Jet.OLEDB.4.0;Data Source=|DataDirectory|\\otel.mdb"))
            {
                ...
                OleDbCommand yoneticiKomut = new OleDbCommand("SELECT * FROM yonetici WHERE email = @Email AND sifre = @Sifre", baglanti);
                yoneticiKomut.Parameters.AddWithValue("@Email", giris_email_textBox.Text);
                yoneticiKomut.Parameters.AddWithValue("@Sifre", giris_sifre_textBox.Text);
                OleDbDataReader yoneticiDr = yoneticiKomut.ExecuteReader();

                OleDbCommand musteriKomut = new OleDbCommand("SELECT * FROM musteri WHERE mail = @Email", baglanti);
                musteriKomut.Parameters.AddWithValue("@Email", email);
                OleDbDataReader musteriDr = musteriKomut.ExecuteReader();
                if (giris_email_textBox.Text == "xx" && giris_sifre_textBox.Text == "xx")
                {
                    // Onay mesajını göster
                    DialogResult onayy = MessageBox.Show("Yönetici hesabına girmek istediğinize emin misiniz?", "Onay", MessageBoxButtons.YesNo);
                    if (onayy == DialogResult.Yes)
                    {
                ...
                else if (string.IsNullOrEmpty(giris_email_textBox.Text) || string.IsNullOrEmpty(giris_sifre_textBox.Text))
                {
                    MessageBox.Show("Gerekli alanları doldurun");
                }
                else if (yoneticiDr.HasRows)
                {
                ...
                else if (musteriDr.HasRows)
                {
                    // Müşteri girişi başarılı
                    musteriDr.Read();
                    ...
                    string adsoyad = musteriDr["ad"].ToString();
            ...
            else
            {
                using (OleDbConnection baglanti = new OleDbConnection("Provider=Microsoft.Jet.OLEDB.4.0;Data Source=|DataDirectory|\\otel.mdb"))
                {
                    ...
                    else
                    {
                        ...
                        // Form1 yüklenirken, uyg_merhaba_label'in Text özelliğini ayarla
                        form1.Load += (s, args) =>
                        {
                            form1.uyg_merhaba_label.Text = "MERHABA " + adsoyad + "! DİLEDİĞİNİZ REZERVASYONU YAPABİLİRSİNİZ!";
                        };
                        // Form1'yi gösterin
                        form1.Show();
        ...
        private void giris_email_textBox_TextChanged(object sender, EventArgs e)
        {
           
        }

        private void giris_girisyap_button_KeyDown(object sender, KeyEventArgs e)
        {
